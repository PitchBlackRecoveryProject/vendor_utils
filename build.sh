...
ghr -t ${GITHUB_TOKEN} -u ${CIRCLE_PROJECT_USERNAME} -r ${CIRCLE_PROJECT_REPONAME} -n "Latest Release" -c ${CIRCLE_SHA1} -delete ${VERSION} ${BUILDFILE}
elif [[ -n $TEST_BUILDFILE ]]
then
ghr -t ${GITHUB_TOKEN} -u ${CIRCLE_PROJECT_USERNAME} -r ${CIRCLE_PROJECT_REPONAME} -n "Latest Release" -c ${CIRCLE_SHA1} -delete ${VERSION} ${TEST_BUILDFILE}
fi
